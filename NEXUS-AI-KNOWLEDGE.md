...
└── macai/                         — 源码目录
    ...
    │   │   ├── ChatView.swift
    │   │   ├── ChatViewModel.swift
    │   │   ├── ChatLogicHandler.swift  — (含 B3 智能路由集成)
    │   │   ├── ChatInputView.swift
    │   │   ├── ChatWindowController.swift — ⭐ v1.5 C4 多窗口
    │   │   ├── BubbleView/             — (A3 气泡重设计: 品牌色+头像+时间戳)
    │   │   └── BottomContainer/        — (含 Prompt 模板按钮)
    │   ├── ChatList/               — 侧边栏聊天列表
    │   ├── Components/             — 通用 UI 组件
    │   │   └── PromptTemplatePickerView.swift — ⭐ v1.5 B4 模板选择器
    │   ├── Preferences/            — 设置页
    │   └── WelcomeScreen/
    ├── Intents/                    — ⭐ v1.0 新增
    │   ├── AskAIIntent.swift
    ...
    └── Utilities/
        ...
        ├── SmartRouter.swift        — ⭐ v1.5 B3 智能路由
        ├── PromptTemplate.swift     — ⭐ v1.5 B4 模板数据+管理器
        ├── NexusServiceProvider.swift — ⭐ v1.2 C1 系统服务
...
**v1.1:**
- B1 多轮 Quick Panel — 支持上下文对话
- B6 对话导出 — Markdown/PDF/HTML
- A4 输入区重设计 — 统一附件预览+快捷操作栏

**v1.2:**
- A1 主题系统 — NexusTheme 品牌色 (#6C5CE7/#4ECDC4) + 语义色 (Sidebar/Chat/Text/Chrome) + 7 种强调色选择
- A2 侧边栏重设计 — Provider 图标徽章 + 相对时间戳 + Pinned/Recent 分段 + 主题背景色
- C1 系统服务 — 右键 "Ask/Summarize/Translate with Nexus AI" (NSServices + Info.plist)

**v1.5:**
- B3 智能路由 — SmartRouter 6 类分类 (code/image/translation/math/search/general) + 自动切换 Provider
- B4 Prompt 模板 — 12 内置模板 + JSON 持久化 + 分类筛选/搜索 + 自建模板 + popover 选择器
- C4 多窗口 — ChatWindowManager 独立窗口 + 右键 "Open in New Window"
...
### NexusTheme 主题系统 (v1.2 A1)
- 品牌色: nexusPurple (#6C5CE7) / nexusCyan (#4ECDC4) + gradient
- 语义色: NexusTheme.Sidebar/Chat/Text/Chrome/QuickPanel — 全部支持 light/dark
- NexusAccentColor: 7 种可选 (purple/cyan/blue/green/orange/pink/red)
- NexusThemeManager: 单例 + @AppStorage 持久化
- providerColor(for:) / providerIcon(for:): 9 个 Provider 独立颜色和 SF Symbol

### SmartRouter 智能路由 (v1.5 B3)
- SmartRouter.shared 单例，@UserDefaultsBacked 持久化开关
- classify(): 中英双语关键词 + 正则 → 6 种 QueryCategory
- resolve(): 根据分类 + 可用 APIServiceEntity + Token 检查 → 最佳 Provider
- 集成点: ChatLogicHandler.applySmartRouting() 在 sendMessage() 前调用
- 路由优先级: code→DeepSeek, image→Gemini, search→Perplexity, translation→DeepSeek

### Prompt 模板系统 (v1.5 B4)
- PromptTemplate: Codable struct (title/content/category/icon/isBuiltIn)
- PromptTemplateManager: JSON 文件存储 (~/Library/Application Support/macai/prompt_templates.json)
- 12 个内置模板，用户可自建/删除
- PromptTemplatePickerView: popover 选择器，分类 chip 筛选 + 搜索
- 入口: QuickActionsView 的 "Templates" 按钮

### 系统服务 (v1.2 C1)
- NexusServiceProvider: NSObject, 3 个 @objc 方法
- Info.plist NSServices 配置 (Ask/Summarize/Translate)
- macaiApp.onAppear → NexusServiceProvider.register()
- 通知: .nexusServiceTextReceived → QuickInputView.onReceive 填入文本
- GENERATE_INFOPLIST_FILE = NO (手动 Info.plist, 避免 Xcode 警告)

### 多窗口支持 (v1.5 C4)
- ChatWindowManager.shared 管理 [UUID: NSWindow] 字典
- openChat(): 创建 NSWindow + NSHostingView(StandaloneChatView)
